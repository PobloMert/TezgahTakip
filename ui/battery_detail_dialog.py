...
from models.bakim import Tezgah  # Son import düzeltmesi

class BatteryDetailDialog(QDialog):
    """Pil değişim detaylarını gösteren ve düzenleme için şifre soran dialog"""
    ...
    def setup_ui(self):
        self.setWindowTitle(f"Pil Değişim Detayı - ID: {self.record.id}")
        ...
        form_layout.addRow("Eksen:", QLabel(self.record.eksen))
        form_layout.addRow("Pil Modeli:", QLabel(self.record.pil_modeli))
        form_layout.addRow("Değişim Tarihi:", QLabel(self.record.tarih))
        form_layout.addRow("Değişim Yapan:", QLabel(self.record.bakim_yapan))
    ...
    def show_edit_dialog(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Pil Değişim Kaydını Düzenle")
        ...
        eksen = QLineEdit(self.record.eksen or "")
        pil_modeli = QLineEdit(self.record.pil_modeli or "")
        yapan = QLineEdit(self.record.bakim_yapan or "")
        
        form_layout.addRow("Tezgah No:", tezgah_combo)
        form_layout.addRow("Eksen:", eksen)
        form_layout.addRow("Pil Modeli:", pil_modeli)
        ...
        def save_changes():
            ...
            try:
                ...
                self.record.eksen = eksen.text()
                self.record.pil_modeli = pil_modeli.text()
                self.record.bakim_yapan = yapan.text()
                
                session.commit()
                QMessageBox.information(dialog, "Başarılı", "Pil değişim kaydı güncellendi.")
